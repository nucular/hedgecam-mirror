...
echo package %APP_PACKAGE%;public class MyDebug {public static final boolean LOG = true;} > "%GEN_DIR%\java\MyDebug.java"

echo:
echo:
echo Creating R.java...
call "%BUILD_TOOLS_PATH%\aapt.exe" package -f -m --auto-add-overlay --min-sdk-version %MIN_SDK% --target-sdk-version %TARGET_SDK% -S "%CD%\res" -S "%GEN_DIR%\res" -J "%CD%\src" -M "%CD%\AndroidManifest.xml" -I "%ANDROID_JAR%" -J "%GEN_DIR%\java"
...
call "%BUILD_TOOLS_PATH%\aapt.exe" package -f --auto-add-overlay --min-sdk-version %MIN_SDK% --target-sdk-version %TARGET_SDK% -M "%CD%\AndroidManifest.xml" -S "%CD%\res" -S "%GEN_DIR%\res" -A "%CD%\assets" -I "%ANDROID_JAR%" -F "%GEN_DIR%\%APP_NAME%_unsigned.apk" %GEN_DIR%\dex
...
echo:
echo Aligning...
call "%ZIPALIGN%" -f 4 "%GEN_DIR%\%APP_NAME%_unaligned.apk" "%CD%\%APP_NAME%_%VERSION%_debug.apk"
...
call "%ADB%" install -r "%CD%\%APP_NAME%_%VERSION%_debug.apk"
